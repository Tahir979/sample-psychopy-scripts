...
expName = 'replication_of_konstantinaou_beal_lavie_exp1b'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\wasd0\\Desktop\\Psychopy\\Konstantinaou, N., Beal, E. J. R. ve Lavie, N. (2013)\\replication_of_konstantinaou_beal_and_lavie_exp1b_lastrun.py',
...
# --- Initialize components for Routine "LA150msL" ---
...
# --- Initialize components for Routine "CA1850ms" ---
PA1850 = visual.ShapeStim(
    win=win, name='PA1850', vertices='cross',units='norm', 
    size=(0.022, 0.04),
    ori=0.0, pos=(0, 0), anchor='center',
    lineWidth=1.0,     colorSpace='rgb',  lineColor='black', fillColor='black',
    opacity=None, depth=0.0, interpolate=True)
...
PD1000 = visual.ShapeStim(
    ...
    lineWidth=1.0,     colorSpace='rgb',  lineColor='black', fillColor='black',
    opacity=None, depth=0.0, interpolate=True)
# Run 'Begin Experiment' code from CD1000
counter = 0

# --- Initialize components for Routine "LD150msL" ---
...
# --- Initialize components for Routine "CD1850ms" ---
PD1850 = visual.ShapeStim(
    win=win, name='PD1850', vertices='cross',units='norm', 
    size=(0.022, 0.04),
    ori=0.0, pos=(0, 0), anchor='center',
    lineWidth=1.0,     colorSpace='rgb',  lineColor='black', fillColor='black',
    opacity=None, depth=0.0, interpolate=True)
...
LoopAlistirma = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('replication_of_konstantinaou_beal_and_lavie_exp1b.xlsx'),
...
for thisLoopAlistirma in LoopAlistirma:
    ...
    while continueRoutine:
        ...
        if PA1000.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    # --- Prepare to start Routine "LA150msL" ---
    continueRoutine = True
    # update component parameters for each repeat
    TALoadL.setText(yuk)
    ...
    if TALoadL.text == "low":
        ...
        if kutu4 == 1:
            ...
            IA9L.size = (0.022, 0.04)
    # keep track of which components have finished
    LA150msLComponents = [TALoadL, IA1L, IA2L, IA3L, IA4L, IA5L, IA6L, IA7L, IA8L, IA9L]
    for thisComponent in LA150msLComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        if IA1L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA2L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA3L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA4L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA5L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA6L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA7L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA8L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA9L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    # --- Ending Routine "LA150msL" ---
    for thisComponent in LA150msLComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "LA150msL" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "CA1850ms" ---
    continueRoutine = True
    # update component parameters for each repeat
    # keep track of which components have finished
    CA1850msComponents = [PA1850]
    for thisComponent in CA1850msComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if PA1850 is starting this frame...
        if PA1850.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
            PA1850.frameNStart = frameN  # exact frame index
            PA1850.tStart = t  # local t and not account for scr refresh
            PA1850.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(PA1850, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'PA1850.started')
            # update status
            PA1850.status = STARTED
            PA1850.setAutoDraw(True)
        
        # if PA1850 is active this frame...
        if PA1850.status == STARTED:
            # update params
            pass
        
        # if PA1850 is stopping this frame...
        if PA1850.status == STARTED:
            if frameN >= 111:
                # keep track of stop time/frame for later
                PA1850.tStop = t  # not accounting for scr refresh
                PA1850.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'PA1850.stopped')
                # update status
                PA1850.status = FINISHED
                PA1850.setAutoDraw(False)
    ...
    # --- Ending Routine "CA1850ms" ---
    for thisComponent in CA1850msComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "CA1850ms" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    # keep track of which components have finished
    FA150msLComponents = [IABackgroundL, TAHedefL, TACeldiriciL]
    ...
    while continueRoutine:
        ...
        if IABackgroundL.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    while continueRoutine:
        ...
        if TAFlankerR.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    while continueRoutine:
        ...
        if IA1R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA2R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA3R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA4R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA5R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA6R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA7R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA8R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if IA9R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
...
for thisLoopDeney in LoopDeney:
    ...
    while continueRoutine:
        ...
        if PD1000.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    if TDLoadL.text == "low":
        ...
        if kutu4 == 1:
            ...
            ID9L.size = (0.022, 0.04)
    # keep track of which components have finished
    LD150msLComponents = [TDLoadL, ID1L, ID2L, ID3L, ID4L, ID5L, ID6L, ID7L, ID8L, ID9L]
    for thisComponent in LD150msLComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        if TDLoadL.status == NOT_STARTED and frameN >= 9:
            # keep track of start time/frame for later
        ...
        if ID1L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID2L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID3L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID4L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID5L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID6L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID7L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID8L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID9L.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    # --- Ending Routine "LD150msL" ---
    for thisComponent in LD150msLComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "LD150msL" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "CD1850ms" ---
    continueRoutine = True
    # update component parameters for each repeat
    # keep track of which components have finished
    CD1850msComponents = [PD1850]
    for thisComponent in CD1850msComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if PD1850 is starting this frame...
        if PD1850.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
            PD1850.frameNStart = frameN  # exact frame index
            PD1850.tStart = t  # local t and not account for scr refresh
            PD1850.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(PD1850, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'PD1850.started')
            # update status
            PD1850.status = STARTED
            PD1850.setAutoDraw(True)
        
        # if PD1850 is active this frame...
        if PD1850.status == STARTED:
            # update params
            pass
        
        # if PD1850 is stopping this frame...
        if PD1850.status == STARTED:
            if frameN >= 111:
                # keep track of stop time/frame for later
                PD1850.tStop = t  # not accounting for scr refresh
                PD1850.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'PD1850.stopped')
                # update status
                PD1850.status = FINISHED
                PD1850.setAutoDraw(False)
    ...
    # --- Ending Routine "CD1850ms" ---
    for thisComponent in CD1850msComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "CD1850ms" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    # keep track of which components have finished
    FD150msLComponents = [IDBackgroundL, TDHedefL, TDCeldiriciL]
    ...
    while continueRoutine:
        ...
        if IDBackgroundL.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    while continueRoutine:
        ...
        if TDFlankerR.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
    ...
    while continueRoutine:
        ...
        if ID1R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID2R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID3R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID4R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID5R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID6R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID7R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID8R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
        ...
        if ID9R.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
